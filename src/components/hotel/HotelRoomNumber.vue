<template>
  <div id="wrap" class="clearfix">
    <h1 class="userClass" style="font-size: 20px;">酒店生成房间管理</h1>
    <el-col :span="24" class="formSearch">
      <el-form :inline="true">
        ...
        <el-form-item  style="float: right;">
          ...
          <el-button type="primary" @click="Add" size="small">生成房间</el-button>
        </el-form-item>
      </el-form>
    </el-col>
...
<script>
  ...
  export default{
    ...
    created(){
      ...
      let roomProductID = sessionStorage.getItem('roomProductID');
      if(!roomProductID){
        this.$notify({
          message: '请先选择房间!',
          position: 'top-left',
      ...
      this.roomProductID = roomProductID
      this.roomID = id;
      this.initData(1)
    },
    methods: {
      ...
      initData(page){
        let options = {
          ...
          "ht_rs_RoomEntityId":this.roomID,//房间实体ID
          "ht_rpp_BeginUseDate": "",//开始供应日期
          "ht_rpp_EndUseDate": "",//结束供应日期
          "page":page?page:1,
          "rows":'5'
        ...
        this.isLoading = true;
        this.$store.dispatch('initHotelRoomNumber',options)
          .then((total) => {
            this.total = total
      ...
      //生成提交
      addSubmit(){
        this.addOptions.ht_rpp_ID = this.roomProductID;
        this.addOptions.ht_rs_RoomEntityId = this.roomID;
        this.addOptions.ht_rpp_BeginUseDate = this.date[0];
        this.addOptions.ht_rpp_EndUseDate = this.date[1];

        this.$store.dispatch('AddHotelRoomNumber',this.addOptions)
          .then(suc => {
            this.$notify({
              message: suc,
              type: 'success'
